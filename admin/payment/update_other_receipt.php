...
if(isset($_GET['id'])) {
    ...
    $receipt_query = mysqli_query($conn, "
        ...
        WHERE r.OtherReceiptID = '$receipt_id'
    ");
...
if(isset($_POST['submit'])) {
    ...
    if (empty($accounting_date) || $accounting_date == '0000-00-00') {
        $accounting_date = date('Y-m-d'); // Sử dụng ngày hiện tại nếu không có giá trị
    }

    // Validate định dạng ngày
    if (!preg_match("/^\d{4}-\d{2}-\d{2}$/", $accounting_date)) {
        $error_msg = "Định dạng ngày không hợp lệ!";
    } else {
        // Cập nhật thông tin phiếu thu
        $update_query = mysqli_query($conn, "
            UPDATE OtherReceipt SET
        ...
        if($update_query) {
            ...
            $receipt_query = mysqli_query($conn, "
                SELECT 
                    ...
                    b.ProjectId,
                    CASE 
                        WHEN r.AccountingDate = '0000-00-00' THEN DATE_FORMAT(NOW(), '%Y-%m-%d')
                        ELSE r.AccountingDate 
...
<body>
                    ...
                    <div class="form-section">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" value="<?php echo number_format($receipt_data['Price']); ?> VNĐ" disabled>
                            </div>
                            <div class="col-md-6">
                                ...
                                <input type="date" class="form-control" name="accounting_date" id="accounting_date"
                                       value="<?php echo isset($receipt_data['AccountingDate']) ? $receipt_data['AccountingDate'] : date('Y-m-d'); ?>"
                                       required
                                       <?php echo (isset($receipt_data['PaymentMethod']) && $receipt_data['PaymentMethod'] == 'Tiền mặt') ? 'disabled' : ''; ?>>
                            </div>
                            <div class="col-12">
    ...
    <script>
        $(document).ready(function() {
            // Kiểm tra hình thức thu để enable/disable ngày hạch toán
            const paymentMethod = '<?php echo $receipt_data['PaymentMethod']; ?>';
            const accountingDateInput = $('#accounting_date');
            
            if (paymentMethod === 'Tiền mặt') {
                accountingDateInput.prop('disabled', true);
            }

            // Kiểm tra giá trị ngày hạch toán
            if (!accountingDateInput.val() || accountingDateInput.val() === '0000-00-00') {
                accountingDateInput.val(new Date().toISOString().split('T')[0]);
            }

            // Validate form trước khi submit
            $('#editReceiptForm').submit(function(e) {
                const accountingDate = accountingDateInput.val();
                if (!accountingDate || accountingDate === '0000-00-00') {
                    e.preventDefault();
                    alert('Vui lòng nhập ngày hạch toán hợp lệ!');
                    return false;
                }
            });
        });
